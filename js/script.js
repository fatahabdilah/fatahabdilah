...
window.addEventListener('load', function () {
    ...
    setTimeout(() => {
        startTypewriter(); // Mulai animasi typewriter setelah loading selesai
    }, 5000);
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', e => {
            ...
            setTimeout(() => {
                targetSection.style.setProperty('--before-height', '300px');
            }, 0); // Sedikit delay untuk memicu transisi
            targetSection.style.display = "grid";
            setTimeout(() => {
                targetSection.style.top = "0";
            }, 100);
...
// Fungsi untuk memulai animasi typewriter
function startTypewriter() {
    var i = 0;
    var txt = 'Front-end Website Developer & UI/UX Designer.';
    ...
    function typeWriter() {
        if (i < txt.length) {
            ...
            blinkingCursor.classList.add("blinking"); // Mengaktifkan animasi kedip setelah selesai mengetik
        }
    }

    typeWriter(); // Memulai pengetikan
}
